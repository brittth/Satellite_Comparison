...
for (row in 1:nrow(data.bc)){
  ...
  else if (data.bc$`Wavelength [nm]`[row] >= 565 && data.bc$`Wavelength [nm]`[row] < 590){data.bc$Color[row] <- 'darkgoldenrod'}
...
## print Band Comparison plot
```{r}
ggsave(file=paste0("Band_Comparison.pdf"), plot=plot.bc, width=15, height=5)
```
